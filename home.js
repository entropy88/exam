...
import {getRecentWikies} from "./data.js";

function homeTemplate(wikis){
    return html`
    <section id="home-page" class="content">
            <h1>Recent Articles</h1>
            ${wikis.map(wikiTemplate)}
...
export async function home(ctx){
    let wikis=await getRecentWikies();
    ctx.render(homeTemplate(wikis))

}

function wikiTemplate(wiki){
    return html`
    <section class="recent js">
                <h2>${wiki.category}</h2>
                <article>
                    <h3>${wiki.title}</h3>
                    <p>${wiki.content}</p>
                    <a href="/details/${wiki._id}" class="btn details-btn">Details</a>
                </article>
            </section>
    `
}